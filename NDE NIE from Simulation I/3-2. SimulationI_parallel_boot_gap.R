...
data=read.csv(paste("sim1_gapdata",as.character(simseed+1),".csv",sep=""),header=TRUE)
...
est=read.table(paste("estIgap",as.character(simseed+1),".txt",sep=""),header=FALSE,row.names=1)
...
estvar=read.table(paste("covIgap",as.character(simseed+1),".txt",sep=""),header=FALSE,row.names=1,na.strings = ".")
...
write.csv(myBest,paste("Boot_gaplres1",as.character(simseed+1),"batch=", as.character(bootbatch),".csv",sep=""),row.names=FALSE)
