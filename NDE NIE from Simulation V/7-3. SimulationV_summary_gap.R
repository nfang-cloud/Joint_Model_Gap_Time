...
for (i in 1:200){
  myBest=NULL
  skip_to_next <- FALSE
  tryCatch({myest1=read.csv(paste("est5_gaplres",as.character(i),".csv",sep=""))}, error = function(e) { skip_to_next <<- TRUE})
  myest=c(myest1$NDE,myest1$NIE)
  for (batch in 1:5){
    for (m in 1:4){
      skip_to_next <- FALSE
      tryCatch({myBest_temp=read.csv(paste("Boot5_gaplres",as.character(i),"batch=",as.character(batch),"M=",as.character(m*5),".csv",sep=""))}, error = function(e) { skip_to_next <<- TRUE})
...
write.csv(rbind(truevalue1,bias,ese,sd,meSE,cr),"sim5gapres_log.csv")
