...
for (mm in 1:MM){
  res=read.csv(paste("Res_Gap_Trt",as.character(1111),"batch=",as.character(mm-1),".csv",sep=""))
...
for (mm in 1:Ba_tmp){
  Bres_tmp=read.csv(paste("Bres_Gap_Trt",as.character(1111+sim-1),"batch=",as.character(mm),".csv",sep=""))
...
cox_fit <- coxph(Surv(fup*30,event)~prevoi+trt+gender+hemobl+stratum+cd4bl, data=surv)
...
###Plot the NDE/NIE/TE ##
pdf("Effects_Gap_Trt_Final.pdf")
plot(c(R_NDE,R_NIE,R_TE)~c(tseq,tseq,tseq),type="n",ylim=c(-0.05,0.15)
     ,ylab="Effect (Difference in Survival Probability)",xlab="Time (days)",pch=24,cex.lab=1.2)
