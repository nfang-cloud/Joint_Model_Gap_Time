...
res=NULL
for (mm in 1:MM){
  res=read.csv(paste("Res_Gap_CD4",as.character(1111),"batch=",as.character(mm-1),".csv",sep=""))
...
for (sim in 1:simseed){
  for (mm in 1:Ba_tmp){
    Bres_tmp=read.csv(paste("Bres_Gap_CD4",as.character(1111+sim-1),"batch=",as.character(mm),".csv",sep=""))
...
pdf("Effects_Gap_CD4_Final.pdf")
plot(c(R_NDE,R_NIE,R_TE)~c(tseq,tseq,tseq),type="n",ylim=c(-0.05,0.25)
