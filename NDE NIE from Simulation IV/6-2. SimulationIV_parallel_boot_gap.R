...
est=read.table(paste("estIVgap",as.character(simseed+1),".txt",sep=""),header=FALSE,row.names=1)
...
estvar=read.table(paste("covIVgap",as.character(simseed+1),".txt",sep=""),header=FALSE,row.names=1,na.strings = ".")
...
write.csv(myBest,paste("Boot4_gaplres",as.character(simseed+1),"batch=", as.character(bootbatch),".csv",sep=""),row.names=FALSE)
